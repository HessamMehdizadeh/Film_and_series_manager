...
void extract_info(string file_name, string& series, int& season, int& episode) {
    ...
    if (regex_search(file_name, match, pattern)) {
        ...
        season = stoi(match[2].str());
        episode = stoi(match[3].str());
    }
}
    ...
    virtual bool OnInit() {
        ...
        for (const auto& entry : fs::recursive_directory_iterator(download_dir)) {
            try {
                ...
                if (series.empty() || season == 0 || episode == 0) { // skip if the file name doesn't match the pattern
                    continue;
                }
                ...
                // log the exception e.what() and the file path, and then continue
                std::cerr << "Exception occurred: " << e.what() << " while processing file: " << entry.path() << std::endl;
                continue;
            }
        }
